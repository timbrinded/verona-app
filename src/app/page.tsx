...
function PhotoCarousel({ place, isOnline }: { place: Place; isOnline: boolean }) {
  const approvedMedia = useMemo(
    () =>
      place.media
        .filter((media) => media.approved && media.url)
  ...
  if (!isOnline || approvedMedia.length === 0) return null;

  const visibleMedia = approvedMedia.filter((media) => !failedUrls.has(media.url));
    ...
    <section className="mt-3" aria-label={`${place.name} photos`}>
      ...
      <div className="flex snap-x gap-2 overflow-x-auto pb-2 [-webkit-overflow-scrolling:touch]">
        {visibleMedia.map((media, index) => {
          const host = media.attribution || photoHost(media.sourceUrl || media.url);
          return (
          <a
            key={media.url}
            href={media.sourceUrl || media.url}
            target="_blank"
          ...
          >
            {!loadedUrls.has(media.url) && (
              <div className="absolute inset-0 flex flex-col justify-end bg-[var(--vb-paper)] p-3 text-black">
                ...
                <div className="mt-0.5 break-words text-lg font-black leading-none">{host}</div>
              </div>
            )}
            ...
            <img
              ...
              className={`h-full w-full object-cover transition duration-200 group-hover:scale-[1.03] ${
                loadedUrls.has(media.url) ? "opacity-100" : "opacity-0"
              }`}
              loading="lazy"
              referrerPolicy="no-referrer"
              onLoad={() => {
                setLoadedUrls((current) => new Set(current).add(media.url));
              }}
              onError={() => {
                setFailedUrls((current) => new Set(current).add(media.url));
              }}
            />
            <div className="absolute bottom-1.5 left-1.5 right-1.5 flex items-center justify-between gap-2 bg-[rgba(255,248,230,0.92)] px-2 py-1 text-[10px] font-black uppercase text-black backdrop-blur-sm">
              <span className="truncate">{media.caption || host}</span>
