...
function rowFromResult(result: ParallelResult): CsvRow {
  ...
  const values: Record<string, string> = {
    ...
    source_category: get(input, ["source_category", "venue_type"]) || get(output, ["source_category", "venue_type"]),
    ...
    booking_url: get(output, ["booking_url", "booking", "ticket_url"]) || get(input, ["booking_url", "booking"]),
    ticket_url: get(output, ["ticket_url"]),
    ...
    social_tiktok: get(output, ["social_tiktok", "tiktok"]),
    ...
    late_open_confidence: get(output, ["late_open_confidence", "late_confidence"]),
    late_hours_evidence: get(output, ["late_hours_evidence"]),
    latest_confirmed_close: get(output, ["latest_confirmed_close"]),
    late_days: stringValue(output.late_days),
    music_style: get(output, ["music_style"]),
    crowd_age_range: get(output, ["crowd_age_range"]),
    crowd_type: get(output, ["crowd_type"]),
    queue_likelihood: get(output, ["queue_likelihood"]),
    queue_duration: get(output, ["queue_duration"]),
    door_policy: get(output, ["door_policy"]),
    busy_level: get(output, ["busy_level"]),
    peak_time: get(output, ["peak_time"]),
    heat_sweat_level: get(output, ["heat_sweat_level"]),
    dancefloor: get(output, ["dancefloor"]),
    last_entry_risk: get(output, ["last_entry_risk"]),
    ...
    nightlife_score_components: get(output, ["nightlife_score_components", "late_night_score_components"]),
